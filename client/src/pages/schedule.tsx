Can we plan an upgrade for this website - Manus

Share
i dont understand the deployment on render, is it done automatically or what
Max
Not automatically yet.
Right now, I prepared the website files so they are ready for Render, but Render will only deploy automatically after you connect the project to Render.
There are two options:
Option
How it works
Automatic updates?
GitHub + Render
Push the website to GitHub, connect that repo to Render
Yes. Every time you push changes, Render rebuilds automatically.
Manual ZIP upload
Upload the built files yourself
No. You must upload again after every change.
Best choice: GitHub + Render.
On Render you create a Static Site, connect the GitHub repo, then use:
Bash
Build command: pnpm install --frozen-lockfile && pnpm build
Publish directory: dist/public
After that, every website update becomes simple: edit code → push to GitHub → Render deploys automatically.
on the orari i nxenesve sent me the file empty of the schedules since the school is over and the information will be updated once the school starts
Max
Got it — I’ll update the student schedule area so it shows an empty/placeholder schedule because the school year is over, and it can be filled again when school starts.



Max
Done — I emptied Orari i Nxënësve.
The page now shows a clean placeholder saying the schedule will be updated when the new school year starts. I removed the old class/teacher schedule data, and the build passes successfully.
schedule.tsx
Code · 5.13 KB
